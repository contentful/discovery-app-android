package discovery.contentful.services;
...
import discovery.contentful.db.DBHelper;
...
public class DBIntentService extends IntentService {
  ...
  private void saveCredentials(Intent intent) {
    ...
    if (StringUtils.isBlank(space) || StringUtils.isBlank(accessToken)) {
    ...
    DBHelper.getInstance()
        .getWritableDatabase()
        .insertWithOnConflict(CredentialsTable.NAME, null, values, SQLiteDatabase.CONFLICT_REPLACE);
  }
}
