...
public class TutorialFragment extends Fragment implements TutorialAdapter.Listener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        tutDialog = ProgressDialog.show(
                getActivity(),
                getString(R.string.pd_tutorial_title),
                getString(R.string.pd_tutorial_message),
                true,
                false);
        ...
        getLoaderManager().initLoader(Utils.getLoaderId(this), null,
                new LoaderManager.LoaderCallbacks<TutorialLoader.Tutorial>() {
                    @Override
                    public Loader<TutorialLoader.Tutorial> onCreateLoader(int i, Bundle bundle) {
                    ...
                    @Override
                    public void onLoaderReset(Loader<TutorialLoader.Tutorial> tutorialLoader) {
                        if (tutDialog != null) {
                            tutDialog.cancel();
                            tutDialog = null;
                        }
    ...
    public void onVideoClicked() {
        videoView.setVideoURI(Uri.parse(
                "android.resource://" +
                        IntentConsts.PACKAGE_NAME + "/" +
                        R.raw.contentful_video));
    ...
    public static boolean handleOnBackPressed(FragmentManager fm) {
        TutorialFragment tutorialFragment = (TutorialFragment) fm
                .findFragmentByTag(TutorialFragment.class.getName());
