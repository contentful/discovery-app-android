...
public class DBHelper extends SQLiteOpenHelper {
     ...
     */
    private static final String DB_NAME = CFApp.getInstance()
            .getResources()
            .getString(R.string.db_name);

    private static final Integer DB_VERSION = Integer.valueOf(CFApp.getInstance()
            .getResources()
    ...
    private void insertStaticData(SQLiteDatabase db) {
        ...
        // Ancient Myths Space
        updateValuesFromStrings(values,
                R.string.demo_space_name,
                R.string.demo_space_key,
                R.string.demo_space_token);

        db.insert(CredentialsTable.NAME, null, values);
    }

    private static void updateValues(ContentValues values,
                                     String spaceName,
                                     String spaceKey,
                                     String accessToken) {
    ...
    private static void updateValuesFromStrings(ContentValues values,
                                                @StringRes int spaceName,
                                                @StringRes int spaceKey,
                                                @StringRes int accessToken) {

        Context context = CFApp.getInstance();

        updateValues(values,
                context.getString(spaceName),
                context.getString(spaceKey),
